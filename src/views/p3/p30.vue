<template>
  <div class="wrapper">
    <van-tabs v-model="active">
      <van-tab name="0" title="待审批"> <list-item type="0" @handlerClick="goToDetails" /></van-tab>
...
export default {
  ...
  methods: {
    goToDetails({ path, item }) {
      setStorage('tab_p31', this.active)
      this.$router.push({ path: path, query: { id: item.id } })
    }
  },
  created() {},
  mounted() {
    const index = getStorage('tab_p31')
    if (index != null) {
      this.active = index
    }
  },
