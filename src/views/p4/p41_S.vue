<template>
  <div>
    <van-row class="partner">
      <van-col span="24">
        <van-cell title="请选择供应商" :value="curPartnerName"></van-cell>
      </van-col>
    </van-row>
    <van-row class="date">
      <van-col span="24">
        <van-cell title="请选择日期" :value="currentDateStr"></van-cell>
      </van-col>
    </van-row>
    <van-row class="root">
  ...
  methods: {
    ...
    onClickSubmit() {
      if (this.invs_p.length <= 0) {
        ...
        this.partners_p.forEach(({ id }) => {
          let t = getStorage(this.groupId + '_P41_ShopCar_' + id)
          dic.push(this.groupId + '_P41_ShopCar_' + id)
          if (t != null && t != '') {
            tmp = tmp.concat(JSON.parse(t))
          }
        })
        setStorage(this.groupId + '_shopCar_P41', JSON.stringify(tmp))
        setStorage(this.groupId + '_shopCar_P41_Dic', JSON.stringify(dic))

        this.$router.push({
          path: '/p41_1_S',
          query: {
            date: this.currentDateStr,
            askDate: this.$route.query.askDate,
            requiredDate: this.$route.query.requiredDate,
            group: this.groupId,
            id: this.id
          }
        })
