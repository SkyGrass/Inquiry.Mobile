        ...
        >
          ...
          <van-cell :label="inv.specification">
            ...
            <template #extra>
              ...
              <!-- <van-stepper
                @plus="onPlus(i + '_' + inv.clsId + '_' + inv.id + '_q', inv)"
                @minus="onMinus(i + '_' + inv.clsId + '_' + inv.id + '_q', inv)"
                v-model="inv.count"
                theme="round"
                integer
                min="0"
    ...
    <van-popup v-model="carVisiable" safe-area-inset-bottom round position="bottom" :style="{ height: '80%' }">
      ...
      <van-cell-group :title="c.clsName" v-for="(c, i) in cars" :key="i">
        <van-cell :label="v.specification" v-for="(v, a) in c.invs" :key="a">
          ...
          <template #label>
            <p class="custom-title">规格:{{ v.specification == '' ? '-' : v.specification }}</p>
            <span class="custom-title">说明:{{ v.remark == '' ? '-' : v.remark }}</span>
          </template>
          <template #extra>
            <van-stepper
              @plus="onPlus(i + '_' + v.clsId + '_' + v.id + '_p', v)"
...
export default {
  ...
  methods: {
    ...
    onChangeCount(value, name, unit, remark) {
    ...
    chooseUnit(name, inv) {
      ...
      this.curUnits = [unitname, unitname2].filter(f => f != null && f != '')
